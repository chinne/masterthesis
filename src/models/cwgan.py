    ...
    for epoch in range(num_epochs):
        ...
        for i, (data, label) in enumerate(dataloader):
            ...
            for iter in range(n_critic):
                ...
                fake_samples = netG(noise, label).detach()
                optimizerD.zero_grad()

                lossD = - torch.mean(netD(real_data, label)) + torch.mean(netD(fake_samples, label))
            ...
            gen_samples = netG(noise, label)
            optimizerG.zero_grad()
            lossG = -torch.mean(netD(gen_samples, label))
            lossG.backward()
            optimizerG.step()
        ...
        if epoch % 10 is 0:
            ...
            torch.save({
                ...
                }, "models/cwgan/generator/generator_{}.pth".format(epoch))
            
            torch.save({
                    "Epochs": epoch,
                    "crictic": netD.state_dict(),
                    "optimizerD": optimizerD.state_dict()
                }, "models/cwgan/critic/critic_{}.pth".format(epoch))
...
def generate_data(epoch: int, randomNoise_dim: int, hidden_dim: int, realData_dim: int, amount: int, device: str):
    if device is 'cpu':
        ...
        Tensor = torch.cuda.FloatTensor
    netG = Generator(randomNoise_dim, hidden_dim, realData_dim).to(device)
    checkpoint = torch.load(f'models/cwgan/generator/generator_{str(epoch)}.pth')
